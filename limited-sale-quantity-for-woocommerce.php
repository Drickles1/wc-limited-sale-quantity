<?php
/**
 * Plugin Name: Limited Sale Quantity for WooCommerce
...
function lsqw_woocommerce_missing_notice() {
    echo '<div class="notice notice-error"><p>'
        . esc_html__( 'Limited Sale Quantity for WooCommerce requires WooCommerce to be installed and active.', 'lsqw' )
        . '</p></div>';
}
...
add_action( 'woocommerce_product_options_pricing', function () {
    ...
    woocommerce_wp_text_input( array(
        ...
        'label'             => __( 'Sale Quantity Allocation', 'lsqw' ),
        'description'       => __( 'Max units to sell at the sale price above. Once that many units sell (by order or stock sync), the sale price is auto-removed, even if stock remains. Leave blank/0 to disable.', 'lsqw' ),
        'desc_tip'          => true,
        'type'              => 'number',
    ...
    woocommerce_wp_checkbox( array(
        'id'          => LSQW_REARM_FIELD,
        'label'       => __( 'Start new limited-sale batch now', 'lsqw' ),
        'description' => __( 'Check this AND save to (re)arm the allocation above. Required every time you want to start or restart a batch — leaving it unchecked on a routine save never touches the counter, even if this same allocation number was already used up before.', 'lsqw' ),
    ) );

    if ( '' !== $remaining && null !== $remaining ) {
        echo '<p class="form-field lsqw-remaining-display">'
            . '<label>' . esc_html__( 'Remaining at Sale Price', 'lsqw' ) . '</label>'
...
add_action( 'woocommerce_variation_options_pricing', function ( $loop, $variation_data, $variation ) {
    ...
    woocommerce_wp_text_input( array(
        ...
        'label'             => __( 'Sale Qty Allocation', 'lsqw' ),
    ...
    woocommerce_wp_checkbox( array(
        ...
        'label'         => __( 'Start new batch now', 'lsqw' ),
        'wrapper_class' => 'form-row form-row-full',
        'description'   => __( 'Check to (re)arm the allocation above.', 'lsqw' ),
    ) );
    if ( '' !== $remaining && null !== $remaining ) {
        echo '<p class="lsqw-remaining-display">' . esc_html__( 'Remaining at sale price:', 'lsqw' ) . ' ' . esc_html( $remaining ) . '</p>';
    }
    echo '</div>';
}, 10, 3 );
...
function lsqw_render_badge() {
    ...
    printf(
        ...
        esc_html( sprintf(
            ...
            _n( 'Only %d left at this price!', 'Only %d left at this price!', $remaining, 'lsqw' ),
            $remaining
        ) )
    );
}
